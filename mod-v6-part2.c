...
* Team member contributions :
        Raj Mishra [NetId: RXM190093] - findUnallocatedInode(), addFreeBlock() - modified, cpin(), cpout(), commenting
        Yu Chen [NetId: YXC190072] - logging, allocateFreeBlockToDir(), allocateNewInodeToDir(), makedir(), initfs modified
        Adithya Iyer [NetId: ASI200000] - main() modified, path_to_inode() (used directly for cd), rm(), process_path(), Testing
...
How to run:
    ...
    3. Execute ./mod
    4. Type in the commands, example set of commands
        4.1. openfs abc
        4.2. initfs 550 10
        4.3. mkdir /user
        4.4. mkdir /user/abc
...
void addFreeBlock(int bNumber){
    ...
    printf("Block Number %d added to free list\n",bNumber);
}
...
int cpin(char* extFile,char* intFile,int inode_curr){
    ...
    for(idx=0;idx<9;idx++){
        if(temp_inode.addr[idx]!=0){
            ...
            for(dir_idx=0;dir_idx<32;dir_idx++){
                ...
                if(temp_dir.inode == -1){
                    free_inode = findUnallocatedInode();
                    lseek(fd,64*(free_inode-1),SEEK_SET);
                    read(fd,&newInode,64);
    ...
    if(fileMade == 0){
        ...
        lseek(fd,64*(free_inode-1),SEEK_SET);
        read(fd,&newInode,64);
        newInode.flags = 32260; //TODO recheck
        newInode.size0 = 0;
        newInode.size1 = st.st_size; //TODO recheck
        newInode.nlinks = 1;
        newInode.actime = (int)time(NULL);
        newInode.modtime = (int)time(NULL);
